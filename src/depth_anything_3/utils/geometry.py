...
def generate_raymaps(views, size=None, use_cam2worlds=True):
    b, v, H, W = views['depthmap'].shape
    device = views['depthmap'].device
    if use_cam2worlds:
        cam2worlds = views['camera_pose']
    else:
        cam2worlds = torch.zeros_like(views['camera_pose'])
        cam2worlds[..., 0, 0] = 1.0
        cam2worlds[..., 1, 1] = 1.0
        cam2worlds[..., 2, 2] = 1.0
        cam2worlds[..., 3, 3] = 1.0

    intr_normed = views['camera_intrinsics'].clone()
    intr_normed[..., 0, :] /= W
    intr_normed[..., 1, :] /= H
    
    # print('intr_normed gt', intr_normed)
    # print('cam2worlds gt', cam2worlds)
...
def generate_raymaps_preds(preds, size=None):
    if isinstance(preds['depth'], list):
        b, v, H, W = preds['depth'][0].shape
        device = preds['depth'][0].device
    else:
        b, v, H, W = preds['depth'].shape
    ...
    intr_normed = preds['intrinsics'].clone()
    intr_normed[..., 0, :] /= W
    intr_normed[..., 1, :] /= H
    
    # print('intr_normed pred', intr_normed.shape)
    # print('cam2worlds pred', cam2worlds.shape)
