...
class DinoVisionTransformer(nn.Module):
    ...
    def _get_intermediate_layers_not_chunked(self, x, n=1, export_feat_layers=[], **kwargs):
        ...
        for i, blk in enumerate(self.blocks):
            ...
            if self.alt_start != -1 and i >= self.alt_start and i % 2 == 1:
                ...
                # )
                x = self.process_attention_checkpoint(
                    x, blk, "global", pos=g_pos, attn_mask=kwargs.get("attn_mask", None)
                )
            else:
                # x = self.process_attention(x, blk, "local", pos=l_pos)
                x = self.process_attention_checkpoint(x, blk, "local", pos=l_pos)
                local_x = x
    ...
    def process_attention_checkpoint(self, x, block, attn_type="global", pos=None, attn_mask=None):
        x = checkpoint.checkpoint(
            self.process_attention, 
            x, block, attn_type, pos, attn_mask, use_reentrant=False)
        return x
