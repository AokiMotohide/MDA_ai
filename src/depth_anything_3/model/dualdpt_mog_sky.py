...
class DualDPTMogSky(nn.Module):
    ...
    def __init__(
        ...
        *,
        ...
        def _make_expert_head(out_dim: int) -> nn.Sequential:
        ...
        self.scratch.output_conv2 = _make_expert_head(output_dim)
        for i in range(2, n_experts + 1):
            setattr(self.scratch, f"output_conv2_{i}", _make_expert_head(output_dim))
        self.scratch.output_conv2_list = [self.scratch.output_conv2] + [
            getattr(self.scratch, f"output_conv2_{i}") for i in range(2, n_experts + 1)
        ]

        self.scratch.output_conv2_sky = _make_expert_head(1)
    ...
    def forward(
        ...
        # print('feats', feats[0][0].shape)
        feats = [feat[0].reshape(B * S, N, C) for feat in feats]
        if chunk_size is None or chunk_size >= S*B:
            ...
            for k, v in out_dict.items():
                if isinstance(v, torch.Tensor):
                    out_dict_new[k] = v.reshape(B, S, *v.shape[1:])
                else:
                    assert isinstance(v, list) and isinstance(v[0], torch.Tensor)
                    out_dict_new[k] = [v[i].reshape(B, S, *v[i].shape[1:]) for i in range(len(v))]
            out_dict = out_dict_new
            return Dict(out_dict)
        out_dicts = []
        for s0 in range(0, B * S, chunk_size):
            s1 = min(s0 + chunk_size, S*B)
        ...
        for k, v in out_dicts[0].items():
            if isinstance(v, torch.Tensor):
                out_dict_new[k] = torch.cat([out_dict[k] for out_dict in out_dicts], dim=0)
                out_dict_new[k] = out_dict_new[k].view(B, S, *out_dict_new[k].shape[1:])
            else:
                assert isinstance(v, list) and isinstance(v[0], torch.Tensor)
                for i in range(len(v)):
                    current_items = [
                        out_dict[k][i] for out_dict in out_dicts
                    ]
                    current_items = torch.cat(current_items, dim=0)
                    if k not in out_dict_new:
                        out_dict_new[k] = []
                    out_dict_new[k].append(current_items.view(B, S, *current_items.shape[1:]))
            
        return Dict(out_dict_new)
    ...
    def _forward_impl(
        ...
        main_mog_list = []
        main_conf_list = []
        assert not self.ordered_head
        for i in range(self.num_experts):
            main_logits = self.scratch.output_conv2_list[i](fused_main)
        ...
        sky_logits = self.scratch.output_conv2_sky(fused_main).permute(0, 2, 3, 1)
        main_mog_list.append(sky_logits[..., -1])
        
        main_mog_list = torch.stack(main_mog_list, dim=-1) # (B, H, W, L)

        main_mog_list_partial = torch.log_softmax(main_mog_list[..., :-1], dim=-1)
        main_mog_list_full = torch.log_softmax(main_mog_list, dim=-1)
        # Sky mask: pixels where the sky component (last channel) is largest among all
        sky_mask = (main_mog_list.argmax(dim=-1) == main_mog_list.shape[-1] - 1).float()

        return {
            self.head_main: main_pred_list,
            f"{self.head_main}_conf": main_conf_list,
            "mog_weight": main_mog_list_partial,
            "mog_weight_full": main_mog_list_full,
            "sky_mask": sky_mask,
        }
