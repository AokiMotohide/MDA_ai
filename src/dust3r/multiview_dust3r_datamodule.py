...
from src.dust3r.datasets_cut3r import *
from src.dust3r.datasets import *
from src.dust3r.datasets_cut3r import get_data_loader, get_data_loader_2dataloader
...
class MultiViewDUSt3RDataModule(LightningDataModule):
    ...
    def __init__(
        ...
        max_num_of_images_per_gpu: int = 32,
        use_dynamic_sampler: bool = True,
        max_sequence_length: Optional[int] = 16,
        min_view_size: int = 4,
    ) -> None:
        ...
        self.max_num_of_images_per_gpu = max_num_of_images_per_gpu
        self.use_dynamic_sampler = use_dynamic_sampler
    ...
    def train_dataloader(self) -> DataLoader[Any]:
        ...
        self.train_loader = get_data_loader(
            ...
            ),  # for DeepSpeed ZeRO-2, for some reason the default fork context doesn't work - it would cause a "cannot allocate memory" error
            persistent_workers=True if self.num_workers_val > 0 else False,
            max_num_of_images_per_gpu=self.max_num_of_images_per_gpu,
            use_dynamic_sampler=self.use_dynamic_sampler,
            max_sequence_length=self.max_sequence_length,
            min_view_size=self.min_view_size,
        )
...
class MultiViewDUSt3RDataModuleTwo(LightningDataModule):

    def __init__(
        self,
        train_datasets: list[str],
        train_datasets_aux: list[str],
        validation_datasets: list[str],
        batch_size_per_device: int = 64,
        batch_size_per_device_val: int = 64,
        num_workers: int = 12,
    ...
    def train_dataloader(self) -> DataLoader[Any]:
        ...
        # Concatenate all train dataset strings into a single string with "+" separator
        train_datasets_concat = " + ".join(self.hparams.train_datasets)
        train_datasets_aux_concat = " + ".join(self.hparams.train_datasets_aux)

        print("Building train Data loader for dataset: ", train_datasets_concat)
        train_loader_main = get_data_loader_2dataloader(
            train_datasets_concat,
            train_datasets_aux_concat,
            batch_size=self.batch_size_per_device,
            num_workers=self.num_workers,
            pin_mem=self.pin_memory,
            shuffle=True,
        ...
        self.train_loader = train_loader_main
        ...
        if hasattr(self.train_loader.dataloader2, "dataset") and hasattr(
            self.train_loader.dataloader2.dataset, "set_epoch"
        ):
            self.train_loader.dataloader2.dataset.set_epoch(0)
        ...
        if hasattr(self.train_loader.dataloader2, "sampler") and hasattr(
            self.train_loader.dataloader2.sampler, "set_epoch"
        ):
            self.train_loader.dataloader2.sampler.set_epoch(0)
        ...
        if hasattr(self.train_loader.dataloader2, "batch_sampler") and hasattr(
            self.train_loader.dataloader2.batch_sampler, "set_epoch"
        ):
            self.train_loader.dataloader2.batch_sampler.set_epoch(0)
