...
import numpy as np
from src.dust3r.datasets_cut3r.base.batched_sampler import (
    BatchedRandomSampler,
    CustomRandomSampler,
    DynamicBatchedRandomSampler,
)
import torch
...
class EasyDataset:
    ...
    def make_sampler(
        ...
        # apply maximum sequence length cap
        if max_sequence_length is not None:
            num_of_views = min(num_of_views, max_sequence_length)
        
        if use_dynamic_sampler:
            if max_num_of_images_per_gpu is None:
                raise ValueError(
                    "max_num_of_images_per_gpu must be provided when use_dynamic_sampler=True"
                )
            return DynamicBatchedRandomSampler(
                self,
        ...
        if batch_size is None:
            raise ValueError(
                "batch_size must be provided when use_dynamic_sampler=False"
            )
        ...
        if world_size > 1:  # modified according to gpt
            ...
            sampler.set_indices(
                distributed_indices
            )  # Assign correct indices to the sampler
