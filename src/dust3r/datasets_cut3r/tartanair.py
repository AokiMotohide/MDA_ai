...
class TartanAir_Multi(BaseMultiViewDataset):
    ...
    def __init__(self, ROOT, non_sequential_readout=False, 
                 max_interval=20, max_interval_test=30, single_scene_name=None, *args, **kwargs):
        self.ROOT = ROOT
        self.video = True
        self.is_metric = True
        ...
        self.single_scene_name = single_scene_name 
        if single_scene_name is not None:
            self.new_ids = []
            for idx in range(len(self.start_img_ids)):
                start_id = self.start_img_ids[idx]
                scene_id = self.sceneids[start_id]
                scene_dir = self.scenes[scene_id]
                if single_scene_name in scene_dir:
                    self.new_ids.append(idx)
            
            print(f'Found {len(self.new_ids)} images for scene {single_scene_name}')

    def _load_data(self):

        if os.path.exists(osp.join(self.ROOT, f'pre-calculated-loaddata-{self.num_views}-v2.pkl')):
            with open(osp.join(self.ROOT, f'pre-calculated-loaddata-{self.num_views}-v2.pkl'), 'rb') as f:
            ...
            with open(osp.join(self.ROOT, f'pre-calculated-loaddata-{self.num_views}-v2.pkl'), 'wb') as f:
                pickle.dump(
    ...
    def _get_views(self, idx, resolution, rng, num_views):
        ...
        if self.single_scene_name is not None:
            idx0 = idx % len(self.new_ids)
            idx = self.new_ids[idx0]
        ...
        for v, view_idx in enumerate(image_idxs):
            ...
            image, depthmap, depth_complete, intrinsics = self._crop_resize_if_necessary2(
                image, depthmap, depth_complete, intrinsics, resolution, rng, info=(scene_dir, img)
            )
