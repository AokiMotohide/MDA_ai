...
from src.stream3r.components.heads.dpt_head_mog import DPTHeadMog
...
class STream3RMog(nn.Module, PyTorchModelHubMixin):
    def __init__(self, img_size=518, patch_size=14, embed_dim=1024, freeze="none"):
        ...
        self.depth_head_mog = DPTHeadMog(dim_in=2 * embed_dim, output_dim=3, activation="exp", conf_activation="expp1")
    ...
    def forward(
        ...
        with torch.autocast(device_type=next(self.parameters()).device.type, dtype=torch.float32):
            ...
            if self.depth_head_mog is not None:
                depth, depth_conf, mog_weight = self.depth_head_mog(
                    aggregated_tokens_list, images=images, patch_start_idx=patch_start_idx
                )
                predictions["depth"] = depth
                predictions["depth_conf"] = depth_conf
                predictions["mog_weight"] = mog_weight
