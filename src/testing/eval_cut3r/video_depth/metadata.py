...
def _find_diode_scan_dirs():
    """Discover DIODE leaf scan directories (each is a multi-frame sequence)."""
    base = "data/cut3r_data/diode/val"
    if not os.path.isdir(base):
        return []
    scan_dirs = set()
    for png in glob.glob(os.path.join(base, "**", "*.png"), recursive=True):
        scan_dirs.add(os.path.relpath(os.path.dirname(png), base))
...
dataset_metadata = {
    "davis": {
        "img_path": "data/cut3r_data/davis/DAVIS/JPEGImages/480p",
        "mask_path": "data/cut3r_data/davis/DAVIS/masked_images/480p",
    ...
    "kitti": {
        "img_path": "data/cut3r_data/kitti_backup/kitti/depth_selection/val_selection_cropped/image_gathered",  # Default path
        "mask_path": None,
        "dir_path_func": lambda img_path, seq: os.path.join(img_path, seq),
        "gt_traj_func": lambda img_path, anno_path, seq: None,
        "traj_format": None,
    ...
    "bonn": {
        "img_path": "data/cut3r_data/bonn/rgbd_bonn_dataset",
        "mask_path": None,
    ...
    "nyu": {
        "img_path": "data/cut3r_data/nyu_v2/val/nyu_images",
        "mask_path": None,
        # Each image file is its own "sequence" (single-frame).
        # seq = filename (e.g. "00001.png"), dir_path_func returns the parent dir.
        "dir_path_func": lambda img_path, seq: img_path,
        "gt_traj_func": lambda img_path, anno_path, seq: None,
        "traj_format": None,
        "seq_list": None,
        "full_seq": True,
    ...
    "diode": {
        "img_path": "data/cut3r_data/diode/val",
        "mask_path": None,
        # seq = relative path to a scan dir (e.g. "indoors/scene_00020/scan_00184")
        "dir_path_func": lambda img_path, seq: os.path.join(img_path, seq),
        "gt_traj_func": lambda img_path, anno_path, seq: None,
        "traj_format": None,
        "seq_list": _find_diode_scan_dirs(),
        "full_seq": False,
        "mask_path_seq_func": lambda mask_path, seq: None,
        "skip_condition": None,
        "process_func": lambda args, img_path: process_diode(args, img_path),
    },
    "scannet": {
        "img_path": "data/cut3r_data/scannetv2",
        "mask_path": None,
    ...
    "sintel": {
        "img_path": "data/cut3r_data/sintel/training/final",
        "anno_path": "data/cut3r_data/sintel/training/camdata_left",
        "mask_path": None,
        "dir_path_func": lambda img_path, seq: os.path.join(img_path, seq),
...
def process_diode(args, img_path):
    scan_dirs = set()
    for png in glob.glob(f"{img_path}/**/*.png", recursive=True):
        scan_dirs.add(os.path.dirname(png))
    for scan_dir in tqdm(sorted(scan_dirs)):
        filelist = sorted(glob.glob(f"{scan_dir}/*.png"))
        rel = os.path.relpath(scan_dir, img_path)
        save_dir = os.path.join(args.output_dir, rel)
        yield filelist, save_dir
