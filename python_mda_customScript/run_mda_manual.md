...
| パス | 内容 |
|---|---|
| `<image-folder>/all_cameras_parameters.json` | 後方互換フィールドを維持した Camera Parameters v2 |
| `<image-folder>/reconstructed_scene.glb` | MDA depth/conf から生成した点群 GLB。`--mask-sky` 指定時のみ sky mask を適用 |

## JSON 形式

`all_cameras_parameters.json` は VGGT 版と同じ Camera Parameters v2 です。
...
```json
{
    "image001.jpg": {
        ...
        "image_transform": {"original_to_processed": [[...], [...], [...]]},
        "scene_transform": {"camera_to_scene": [[...], [...], [...], [...]]},
        "provenance": {"provider": "mda", "model_id": "..."}
    }
}
```
...
- 歪みを推定しない現行経路は `distortion.model = "none"` を明示します。将来は同じ項目へ係数を追加できます。
- `image_transform` は元画像からMDA処理画像までのresize/cropを表します。
- `scene_transform.camera_to_scene` は同時生成したGLB座標への直接変換です。
...
GLB export の alignment は VGGT `visual_util.py` と同じ
`inv(E0) * C_gl * R_y180` です。
`all_cameras_parameters.json` には raw OpenCV world の World-to-Camera
extrinsics を出し、アプリ側の VGGT 読み込み処理が同じ alignment を
カメラ側へ適用できるようにします。DA3 標準 GLB export の点群中心合わせは、
VGGT JSON/GLB 互換性を壊すためこのカスタムスクリプトでは使いません。
