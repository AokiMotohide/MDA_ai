...
Loading is on-the-fly from per-frame `depth/`, `color/`, `camera/` files (plus
optional `conf/` and `sky/` — sky pixels are encoded as conf=0 internally), or
...
Usage:
    conda activate mda
    python view.py --data_dir eval_results/demo/<run_name>/<model_name>
    python view.py --data_dir eval_results/demo/<run_name>
"""
...
# Inlined I/O + geometry helpers so this viewer is self-contained in this repo.
...
def load_frame(data_dir, frame_id):
    """Load one frame's depth, conf, color, pose (C2W), intrinsics.

    conf/ is optional (newer demo runs don't store it) — missing conf is
    synthesized as ones. Sky pixels (optional sky/*.npy bool masks) are
    encoded as conf=0 so the viewer's "Show sky" toggle can filter on it.
    """
    tag = f"{frame_id:06d}"
    depth = np.load(os.path.join(data_dir, "depth", f"{tag}.npy"))
    conf_path = os.path.join(data_dir, "conf", f"{tag}.npy")
    if os.path.isfile(conf_path):
        conf = np.load(conf_path)
    else:
        conf = np.ones_like(depth, dtype=np.float32)
    sky_path = os.path.join(data_dir, "sky", f"{tag}.npy")
    if os.path.isfile(sky_path):
        conf = conf.copy()
        conf[np.load(sky_path).astype(bool)] = 0.0
    ...
    if pose.shape == (3, 4):
        ...
        pose_4x4[:3, :] = pose
        pose = pose_4x4
    return {"depth": depth, "conf": conf, "color": color, "pose": pose,
            "intrinsics": cam["intrinsics"]}


def depth_to_cam_coords_points(depth_map, intrinsic):
    """Unproject an (H, W) depth map to (H, W, 3) camera-frame points (OpenCV pinhole)."""
    H, W = depth_map.shape
...
def main():
    ...
    parser.add_argument(
        ...
        type=float,
        default=-1.0,
        help="Conf threshold applied at load. Default -1 keeps everything, including "
        "sky points stored with conf=0 (needed for the 'Show sky' toggle).",
    )
    parser.add_argument(
    ...
    with server.gui.add_folder("Display"):
        gui_n_points = server.gui.add_slider(
            ...
            max=20000,
            step=10,
            initial_value=max(10, min(args.initial_max_points // 100, 1500)),
        )
        ...
        # Sky pixels are stored with conf=0 and depth pushed to 2x the scene's
        # max valid depth; this toggle includes/excludes them in the cloud.
        gui_show_sky = server.gui.add_checkbox("Show sky (2x max depth)", initial_value=False)
    ...
    def _refresh_pointcloud():
        ...
        # Sky points carry conf=0 (depth already at 2x scene max); the toggle
        # decides whether they are part of the displayed cloud.
        if gui_show_sky.value:
            mask = np.ones(len(conf_all), dtype=bool)
        else:
            mask = conf_all > 0.0
        pts = pts_all[mask]
        cols = cols_all[mask]
        ...
        sky_tag = "+sky" if gui_show_sky.value else "no sky"
        gui_status.value = f"[{state['name']}] {label}: {len(pts_ds):,} / {len(pts):,} pts ({sky_tag})"
    ...
    def _load_selected():
        ...
        _refresh_pointcloud()
        _refresh_cameras()
        # Start viewing from the first frame's camera instead of viser's default pose.
        for client in server.get_clients().values():
            try:
                _goto_frame(client, 0)
            except Exception:
                pass
    ...
    # and start the client at the current frame's camera instead of viser's default pose.
